...
text_prg = """
__kernel void acceleration(__global double *m,
                  __global double *r, 
                  __global double *a,
                  const double G,
                  const int N)
...
def OpenCLSolver(m, r, v, T, k):
    N = m.shape[0]
    res_r = np.zeros((N, 2, k))
    res_v = np.zeros((N, 2, k))
    dt = T / (k - 1)
    cur_a = accelerationOpenCL(m, r.flatten())
    res_r[:, : , 0] = r
    res_v[:, : , 0] = v
    for i in range(1, k):
        r = r + v * dt + 0.5 * cur_a * (dt ** 2)
        next_a = accelerationOpenCL(m, r.flatten())
